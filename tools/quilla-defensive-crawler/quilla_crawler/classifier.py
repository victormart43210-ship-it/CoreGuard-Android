...
_CATEGORY_RULES: list[tuple[re.Pattern[str], str]] = [
    ...
    (
        re.compile(r"\b(stalkerware|stalkware|monitoring\s+app|spouseware)\b", re.I),
        "crawler-stalkerware",
    ),
    # Spyware
    (
        re.compile(
            r"\b(spyware|pegasus|nso\s+group|paragon|cytrox|predator|hermit|triton)\b", re.I
        ),
        "crawler-spyware",
    ),
    # MITRE Mobile techniques
    (
        re.compile(r"\b(T\d{4}(\.\d{3})?|mitre|att&ck|mobile\s+technique)\b", re.I),
        "crawler-mitre-mobile",
    ),
    # Android hardening
    (
        re.compile(
            r"\b(hardening|security\s+baseline|scap|benchmark|cis\s+android|masvs|mastg)\b", re.I
        ),
        "crawler-android-hardening",
    ),
    # Incident response
    (
        re.compile(r"\b(incident\s+response|forensic|mvt|ioc\s+collection|artifact)\b", re.I),
        "crawler-incident-response",
    ),
    # Network defense
    (
        re.compile(
            r"\b(network\s+defense|firewall|ids|ips|dns\s+security|tls\s+inspection)\b", re.I
        ),
        "crawler-network-defense",
    ),
    # IOC context
    (
        re.compile(
            r"\b(ioc|indicator\s+of\s+compromise|hash|sha256|md5|ip\s+address|domain\s+ioc)\b", re.I
        ),
        "crawler-ioc-context",
    ),
    # Malware
    (
        re.compile(r"\b(malware|trojan|ransomware|rootkit|botnet|rat|banker|dropper)\b", re.I),
        "crawler-malware",
    ),
    # Vulnerability (default for CVE/KEV)
    (
        re.compile(r"\b(cve|vulnerability|exploit|patch|zero-day|zero\s+day|rce|lpe)\b", re.I),
        "crawler-vulnerability",
    ),
]
