...
4. **Replace** `DemoBillingProvider` with a completed `PlayBillingProvider` in
   `CoreGuardApp` (single process-wide provider). Keep UI copy honest until
   Play Billing + server verification are live.
