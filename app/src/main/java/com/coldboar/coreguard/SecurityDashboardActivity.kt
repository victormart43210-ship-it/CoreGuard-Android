    ...
    private fun renderResults(results: List<SecurityCheckResult>) {
        ...
        results.forEachIndexed { index, result ->
            val card = inflater.inflate(
                ...
                binding.checksContainer,
                false
            ) as MaterialCardView

            val (stateLabel, stateColor, stateDim) = when (result.state) {
                SecurityCheckState.PASS -> Triple(
                    R.string.check_state_pass,
                    getColor(R.color.status_pass),
                    getColor(R.color.status_pass_dim)
                )
                SecurityCheckState.WARN -> Triple(
                    R.string.check_state_warn,
                    getColor(R.color.status_warn),
                    getColor(R.color.status_warn_dim)
                )
                SecurityCheckState.FAIL -> Triple(
                    R.string.check_state_fail,
                    getColor(R.color.status_fail),
                    getColor(R.color.status_fail_dim)
                )
            }
