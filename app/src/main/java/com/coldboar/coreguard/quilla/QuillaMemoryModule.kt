...
/**
 * Module-pattern façade for **Quilla Memory**, research sync, and the
 * Nemesis→choir bridge.
 *
 * UI, [com.coldboar.coreguard.mvt.ScannerModule], VPN, and telemetry should call
 * **this** object — not [QuillaCorrelationEngine], [QuillaIocBridge], or
 * [AngelicDefenseBlessings] internals — so `:feature:quilla` can extract later
 * without rewriting screens (see `docs/MODULE_ARCHITECTURE.md`).
 *
 * ## What this module owns
 *
 * | Concern | Hidden behind façade | Public entry |
 * |---------|----------------------|--------------|
 * | Memory snapshot + choir seal | History, shield, hypotheses, blessings | [memorySnapshot] |
 * | Nemesis scan bridge | IOC correlate, DTS, journal, swarm | [onScanCompleted] |
 * | Hypothesis / correlator | Shared store + engine | [hypothesisStore] / [correlationEngine] |
 * | Research / Infinity | Network sync + local codex train | [syncResearch] / [trainInfinityLocal] |
 *
 * ## Honesty
 *
 * - Research sync ≠ Premium Nemesis signature refresh.
 * - Scan bridge carries evidence only (no lore inventing).
 * - Infinity training is on-device teaching, not a cloud LLM.
 */
object QuillaMemoryModule {
